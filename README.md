...
1. **loadContent**: Reads glossary JSON file from the configured path
2. **contentLoaded**: Creates data files for components and remark plugin, adds glossary page route
3. **getThemePath**: Exposes theme components (`GlossaryTerm`)
4. **getPathsToWatch**: Watches glossary file for changes during development
5. **postBuild**: Optional post-build hook for additional processing
